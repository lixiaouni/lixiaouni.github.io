...
//来源参数名字
    ...
    (function() {
        ...
        // @return {Object}
        //name和expression分别对应device和os等数组
        ...
        // 解析 UserAgent 字符串 window.useragent+window.appversion+window.vender
        // @param {String} ua, userAgent string.
        // @return {Object}
...
// 标准广告系列来源
    ...
    (function() {
        ...
        var each = _.each = function(obj, iterator, context) {
            ...
            } else if (obj.length === +obj.length) {//什么用法？
        ...
        if (_.isObject(o)) {
            _.each(o, function(a, b) {
                if (_.isObject(a)) {
                    ...
                    if (_.isDate(a)) {
                        o[b] = _.formatDate(a);//2016-10-08 15:29:12.233444
                    }
                }
            });
        }
...
// 数组去重复
    ...
    _.UUID = (function() {
        var T = function() {//时间
            var d = 1 * new Date()//unix timetamp ms
                , i = 0;
            while (d == 1 * new Date()) {
                i++;
            }
    ...
    })();//UUID end  157a8398e6c341-0e8f392b53da79-3f7f055c-2073600-157a8398e6d3a9
    ...
    _.urlParse = function(para){
        var URLParser = function (a) {
            this._fields = {
                Username: 4,
                Password: 5,
                Port: 7,                //因为URLParser作为构造函数，所以此处this指URLParser
                Protocol: 2,
                Host: 6,
                Path: 8,
                URL: 0,
...
// 取host  https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Date/setTime
...
// 是否有标准的浏览器环境,如果不是发送$errorEnviroment:{$errorReson:'没有window'}
    ...
    _.cookie = {
        ...
        set: function(name, value, days, cross_subdomain, is_secure) {//路径为根
    ...
    _.xhr = function (cors) {//cors跨域   _.xhr为跨域xmlhttprequest或非跨域xmlhttprequest
        if (cors){
    ...
    _.ajax = function(para){
        function getJSON(data){//由字符串得到javascript对象
            try{
    ...
    _.info = {
        initPage: function(){
            ...
            var referrer_domain = _.getDomainByHost(referrer_domain);//bug bug bug 此处参数应该是referrer_host
...
// 检查是否是新用户（第一次种cookie，且在8个小时内的）
    var saNewUser = {
        ...
        storeInitCheck: function(){
            // 如果是新用户，种cookie//cookie不会超过24:00
            if(is_first_visitor){
                var date = new Date();
                var obj = {
                    h: 23 - date.getHours(),
    ...
    saEvent.send = function(p, callback) {
        ...
        if (!p.type || p.type.slice(0, 7) !== 'profile') {
            // 传入的属性data.properties > 当前页面的属性info.currentProps > session的属性 store.getSessionProps()> cookie的属性store.getProps() > 预定义属性info.properties()
            data.properties = _.extend({}, _.info.properties(), store.getProps(), store.getSessionProps(), _.info.currentProps, data.properties);
        }
        // 如果$time是传入的就用，否则使用服务端时间?使用client时间
        ...
        _.searchObjDate(data);//只会将其中的date对象转换为特定格式
        _.searchObjString(data);//将data中string类型的数据限制长度
        //判断是否要给数据增加新用户属性
    ...
    var store = sd.store = {
        ...
        toState: function(ds) {//ds json字符串  初始化state
            var state = null;
        ...
        sessionSave: function(props) {
            ...
            _.cookie.set('sensorsdata2015session', JSON.stringify(this._sessionState), 0);//关闭浏览器后消失
        },
        save: function() {
            _.cookie.set('sensorsdata2015jssdkcross', JSON.stringify(this._state), 730, sd.para.cross_subdomain);//存两年
        },
        _sessionState: {},
        _state: {},
        init: function() {
            // 如果不支持cookie，设置新的id，并且带有error_msg
            if (!navigator.cookieEnabled) {//浏览器是否启用了cookie
